...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        tokenized_lines = tokenizer.batch_encode_plus(
        ...
        end_of_prompt_ids = []
        for end_of_prompt_token in args.end_of_prompt_tokens:
            end_of_prompt_id = tokenizer.convert_tokens_to_ids(
                tokenizer.tokenize(end_of_prompt_token))[0]
            end_of_prompt_ids.append(end_of_prompt_id)

        # separate x and y
        separated_lines = []
        skipped = 0
        for line in tokenized_lines:
            found_end_of_prompt = False
            for end_of_prompt_id in end_of_prompt_ids:
                try:
                    cutoff = get_last_occurrence(line, end_of_prompt_id)
                    found_end_of_prompt = True
                    break
                except:
                    continue

            if found_end_of_prompt:
                x, y = line[:cutoff+1], line[cutoff+1:]
                separated_lines.append([x, y])
            else:
                # print('NO END OF PROMPT TOKEN FOUND')
                # print(lines[i])
                # print(line)
                # print()
                skipped += 1

        print('SKIPPED BECAUSE MISSING END OF PROMPT:', skipped)

        # pad before x and after y
        padded_lines = []
        shortened = 0
        target_too_long = 0
        source_too_long = 0
        for line in separated_lines:
            if block_size == 1024:  # skip if too long for full recipe model
                if len(line[0]) > args.divider:
                    continue
                if len(line[1]) > block_size - args.divider:
                    continue

            # target has to be smaller than the space to the right of divider
            if len(line[1]) < block_size - args.divider:
                # source has to be smaller than left side - if not, shorten it
                if len(line[0]) > args.divider:
                    inst_token = tokenizer.convert_tokens_to_ids(
                        tokenizer.tokenize('<inst>'))[0]
                    for _ in range(line[0].count(inst_token)):
                        # remove the first instruction
                        # find text between any special token and the first <inst>
                        special_token_cutoff = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(tokenizer.eos_token))[0]
                        for i, tok in enumerate(line[0]):
                            if tok == inst_token:
                                end = i
                                break
                        for i, tok in reversed(list(enumerate(line[0][:end]))):
                            if tok >= special_token_cutoff:
                                start = i
                                break
                        line[0] = line[0][:start+1] + line[0][end+1:]
                ...
                newline_x = [tokenizer.pad_token_id] * args.divider
                newline_x[-len(line[0]):] = line[0]
                newline_y = [tokenizer.pad_token_id] * (block_size - args.divider)
                newline_y[:len(line[1])] = line[1]
                newline = newline_x + newline_y
                padded_lines.append(newline)
            else:
                target_too_long += 1
                continue

        self.examples = padded_lines

        print('EXAMPLES SHORTENED:', shortened)
        print('TARGET TOO LONG:', target_too_long)
        print('SOURCE TOO LONG:', source_too_long)
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter(log_dir=os.getenv('PT_OUTPUT_DIR', '.'))
...
def main():
    ...
    parser.add_argument("--divider", type=int, default=350, help="Which token separates x and y")
    ...
    if 'next_step_train' in args.train_data_file:
        args.divider = int(args.block_size/1.25)
        args.end_of_prompt_tokens = ['<inst>', '<endofings>']
    if 'next_ing_' in args.train_data_file:
        args.divider = int(args.block_size/1.25)
        args.end_of_prompt_tokens = ['<endofinst>']
    if args.train_data_file.startswith('style_transfer_') and 'ing' not in args.train_data_file:
        args.divider = int(args.block_size/1.4)
        args.end_of_prompt_tokens = ['<target:vegan>', '<target:non-vegan>',
                                     '<target:vegetarian>', '<target:non-vegetarian>',
                                     '<target:dairy-free>', '<target:non-dairy-free>',
                                     '<target:egg-free>', '<target:non-egg-free>',
                                     '<target:fish-free>', '<target:non-fish-free>',
                                     '<target:alcohol-free>', '<target:non-alcohol-free>',
                                     '<target:nut-free>', '<target:non-nut-free>']
    if 'style_transfer_ing_' in args.train_data_file:
        args.divider = int(args.block_size/1.4)
        args.end_of_prompt_tokens = ['<endofprompt>']
    if 'next_step_style_transfer_train' in args.train_data_file:
        args.divider = int(args.block_size/1.4)
        args.end_of_prompt_tokens = ['<inst>', '<endofings>']
    if 'next_step_style_transfer_multi_train' in args.train_data_file:
        args.divider = int(args.block_size/1.4)
        args.end_of_prompt_tokens = ['<inst>', '<endofings>']
    if 'next_step_style_transfer_simple_' in args.train_data_file:
        args.divider = int(args.block_size/1.4)
        args.end_of_prompt_tokens = ['<endoftitle>']
    if 'full_recipe_' in args.train_data_file:
        args.divider = int(args.block_size/2)
        args.end_of_prompt_tokens = ['<endofings>']

    args.train_data_file = os.getenv('PT_DATA_DIR', 'data') + '/' + args.train_data_file
    ...
    special_tokens_dict = {'bos_token': '<|startoftext|>',
                           ...
                           'additional_special_tokens': [
                               ...
                               '<endofrecipe>', '<endofprompt>', '<endofcontext>',
                               '<source:rec>', '<source:az>', '<source:se>',
